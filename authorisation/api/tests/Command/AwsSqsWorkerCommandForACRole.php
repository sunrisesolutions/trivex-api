...
class AwsSqsWorkerCommandForACRole extends WebTestCase
{
    ...
    public function ACRolePost() {
        ...
        $acRoleRepo = static::$container->get('doctrine')->getRepository(ACRole::class);
        $acrole = $acRoleRepo->findOneBy(['uuid' => $authAr['uuid']]);
        $this->assertNotEmpty($acrole);
    }

    public function ACRolePut() {
        $acRoleRepo = static::$container->get('doctrine')->getRepository(ACRole::class);
        ...
        $new = $acRoleRepo->find($acrole->getId());
        $this->assertNotEmpty($new);
        $this->assertEquals('ROLE_ORG_ADMIN', $new->getName());
    }

    public function testACRoleRemove() {
        $acRoleRepo = static::$container->get('doctrine')->getRepository(ACRole::class);
        $acrole = $acRoleRepo->findOneBy([], ['id' => 'DESC']);
        $this->assertNotEmpty($acrole);
